...
binance_client.futures_change_leverage(symbol='MATICUSDT', leverage=20)
...
#binance_client.futures_symbol_ticker(symbol='MATICUSDT')
...
#binance_client.cancel_orders(symbol='MATICUSDT')

#binance_client.futures_cancel_all_open_orders(symbol='MATICUSDT')

with urllib.request.urlopen("http://127.0.0.2/prog/bb1matic.csv") as url:
...
with urllib.request.urlopen("http://127.0.0.2/prog/bb2matic.csv") as url:
    buyx2 = url.read().decode('utf-8')
         
    buyid2 = int(buyx2)  

with urllib.request.urlopen("http://127.0.0.2/prog/ss1matic.csv") as url:
...
with urllib.request.urlopen("http://127.0.0.2/prog/ss2matic.csv") as url:
...
if op2>zero and op2!=zeropoint and buyid==buyid2:
    ...
    quantity = opbuy  # Number of coins you wish to buy / sell, float
        ...
        with open(r'C:/xampp/htdocs/prog/bb2matic.csv', 'w') as out_file:
   ...
   closePosition=True
    ...
    quantity = opq  # Number of coins you wish to buy / sell, float
        ...
        reader = csv.reader(buffer, skipinitialspace=True)
        with open(r'C:/xampp/htdocs/prog/ss2matic.csv', 'w') as out_file:
            writer = csv.writer(out_file)
            writer.writerows(reader)
...
if closeminus2 > openminus2 and closeminus1 < openminus1 and openminus1>ema2:
    ...
    #binance_client.futures_cancel_all_open_orders(symbol='MATICUSDT')
    buyara = binance_client.futures_create_order(
      symbol='MATICUSDT',
      type='STOP_MARKET',
      timeInForce='GTC',  # Can be changed - see link to API doc below
      stopPrice=closeminus2,  # The price at which you wish to buy/sell, float
      side='BUY',  # Direction ('BUY' / 'SELL'), string
      quantity=50  # Number of coins you wish to buy / sell, float
      )

    
    buyid = buyara["orderId"]
    buyid2 = buyara["orderId"]
    
    mybuyidstring = str(buyid)
    mybuyidstring
    bb1 = mybuyidstring
    from io import StringIO
    ...
    reader = csv.reader(buffer, skipinitialspace=True)
    with open(r'C:/xampp/htdocs/prog/bb1matic.csv', 'w') as out_file:
        writer = csv.writer(out_file)
        writer.writerows(reader)
...
if closeminus2 < openminus2 and closeminus1 > openminus1 and openminus1<ema2:
   ...
   # binance_client.futures_cancel_all_open_orders(symbol='MATICUSDT')
    sellara = binance_client.futures_create_order(
      symbol='MATICUSDT',
      type='STOP_MARKET',
      timeInForce='GTC',  # Can be changed - see link to API doc below
      stopPrice=closeminus2,  # The price at which you wish to buy/sell, float
    ...
    sellid = sellara["orderId"]
    sellid2 = sellara["orderId"]
    ...
    reader = csv.reader(buffer, skipinitialspace=True)
    with open(r'C:/xampp/htdocs/prog/ss1matic.csv', 'w') as out_file:
        writer = csv.writer(out_file)
        writer.writerows(reader)
