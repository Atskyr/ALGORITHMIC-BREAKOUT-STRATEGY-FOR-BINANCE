...
binance_client.futures_change_leverage(symbol='ETCUSDT', leverage=20)
...
#binance_client.futures_symbol_ticker(symbol='ETCUSDT')
...
#binance_client.cancel_orders(symbol='ETCUSDT')

#binance_client.futures_cancel_all_open_orders(symbol='ETCUSDT')

with urllib.request.urlopen("http://127.0.0.2/prog/bb1etc.csv") as url:
...
with urllib.request.urlopen("http://127.0.0.2/prog/bb2etc.csv") as url:
    buyx2 = url.read().decode('utf-8')
         
    buyid2 = int(buyx2)  

with urllib.request.urlopen("http://127.0.0.2/prog/ss1etc.csv") as url:
...
with urllib.request.urlopen("http://127.0.0.2/prog/ss2etc.csv") as url:
...
if op2>zero and op2!=zeropoint and buyid==buyid2:
    ...
    quantity = opbuy  # Number of coins you wish to buy / sell, float
        ...
        with open(r'C:/xampp/htdocs/prog/bb2etc.csv', 'w') as out_file:
   ...
   closePosition=True
    ...
    quantity = opq  # Number of coins you wish to buy / sell, float
        ...
        reader = csv.reader(buffer, skipinitialspace=True)
        with open(r'C:/xampp/htdocs/prog/ss2etc.csv', 'w') as out_file:
            writer = csv.writer(out_file)
            writer.writerows(reader)
...
if closeminus2 > openminus2 and closeminus1 < openminus1 and openminus1>ema2:
    ...
    buyara = binance_client.futures_create_order(
      ...
      side='BUY',  # Direction ('BUY' / 'SELL'), string
      quantity=5.00  # Number of coins you wish to buy / sell, float
      )
    ...
    reader = csv.reader(buffer, skipinitialspace=True)
    with open(r'C:/xampp/htdocs/prog/bb1etc.csv', 'w') as out_file:
        writer = csv.writer(out_file)
        writer.writerows(reader)
...
if closeminus2 < openminus2 and closeminus1 > openminus1 and openminus1<ema2:
   ...
   # binance_client.futures_cancel_all_open_orders(symbol='ETCUSDT')
    ...
    reader = csv.reader(buffer, skipinitialspace=True)
    with open(r'C:/xampp/htdocs/prog/ss1etc.csv', 'w') as out_file:
        writer = csv.writer(out_file)
        writer.writerows(reader)
